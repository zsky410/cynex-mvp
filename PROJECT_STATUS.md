...
- Current phase: Phase 3 — Admin catalog (Phase 3C code-complete and locally accepted).
...
Review and merge Phase 3C, then complete real staging Category CRUD acceptance before beginning Phase 3D Product work. Public Storefront work remains deferred.
...
- Phase 3C work is on `feature/phase-3c-category-crud`, created from `develop` after Phase 3B acceptance merge commit `a9a20dc`.
...
- Phase `3B` is closed and its staging acceptance record is merged.
- Phase `3C` implements Category list/search/status filter, create/edit, hide/restore, guarded delete, transactional move up/down, Vietnamese slug normalization, mirrored validation, and Vietnamese constraint feedback under the verified Admin action/Origin/RLS boundary.
- Phase `3C` local acceptance passes 16 Vitest assertions, 62 pgTAP assertions, and 12 disposable-user HTTP/RLS checks including three-row create/reorder, duplicate slug, edit, hide/restore, referenced-delete denial, invalid Origin denial, non-admin denial, and complete fixture cleanup. The reorder migration was dry-run, applied, linted, and type-synchronized on staging only. No Product CRUD or Storefront feature is included.
- After merge/deploy, the real staging Admin must repeat the Phase 3C acceptance journey and remove all marked fixtures before Phase 3D begins.
